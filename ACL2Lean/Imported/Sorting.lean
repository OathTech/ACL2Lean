...
-- (`relExec_t_or_nil` DELETED: audit-driven DEAD-CODE DELETION (A4 #8,
-- ruled 2026-08-16) — declaration-only across the whole branch history
-- (no consumer at a65a0c6~1 either); it survived only as a shape-gate
-- REGISTERED SUPPORT entry, and that gate was itself demoted to a census
-- in the 2026-08-11 gate-cruft review. NOT industrialization.)
...
-- (`allRelExec_t_or_nil` DELETED: audit-driven DEAD-CODE DELETION
-- (A4 #8, ruled 2026-08-16) — its last two consumers were the `dis_`
-- dischargers removed by the 2026-08-11 THIN-LEAN PURGE (a65a0c6) and
-- `dis_rule_orderedp_append`, removed by P5a (1becbc5, 2026-08-16).
-- Two-valuedness for ALL-REL now arrives from ACL2's own emitted
-- `:TYPE-PRESCRIPTION` corollary by replay. NOT industrialization.)
...
/-! ## The ordinal kit (`total:O<`) — DELETED.

(`oFinpT`/`oFirstExptT`/`oFirstCoeffT`/`oRstT`/`oLtT`, their bodies and
syms, `oFinpExec`/`oFirstExptExec`/`oFirstCoeffExec`/`oLtExec`, and
`o_finp_exec_corr` / `o_fe_exec_corr` / `o_fc_exec_corr` /
`o_lt_exec_corr` — DELETED: audit-driven DEAD-CODE DELETION (A4 #8,
ruled 2026-08-16) — orphaned by P3b, which retired `total:O<` /
`total:O-P` to the driver's admission prover over ACL2's own emitted
ground-zero `:TERMINATION-CLAUSES` (the ordinal registry row in
`Replay/OrdinalSim`) and deleted `dis_o_lt_total` — see its own record
immediately below — leaving the kit with zero use sites. NOT
industrialization: these were waypoint-layer HAND proofs, so the
waypoint-metrics hand-line count moves here as a DELETION.) -/
...
/-! ## The `acl2-count` kit — DELETED.

(`integerAbsT`/`lengthT`/`acl2CountT`, their bodies and syms,
`integerAbsExec`/`lengthExec`/`acl2CountExec`, and
`integer_abs_exec_corr` / `length_exec_corr` / `acl2_count_exec_corr`
— DELETED: audit-driven DEAD-CODE DELETION (A4 #8, ruled 2026-08-16)
— orphaned by phase 1 of the T1+2 sprint (the D-A ts-algebra consumer,
2026-08-14), which retired `dis_acl2_count_tp` — see its own record
immediately below — leaving the kit with zero use sites. NOT
industrialization: these were waypoint-layer HAND proofs, so the
waypoint-metrics hand-line count moves here as a DELETION.) -/
...
-- (`orderedpExec`, `orderedpExec_t_or_nil`, `orderedp_ns` and
-- `orderedp_exec_corr` — the generic ORDEREDP exec closure — DELETED:
-- audit-driven DEAD-CODE DELETION (A4 #8, ruled 2026-08-16) — orphaned
-- by P5a (1becbc5, 2026-08-16), whose `routeIff` REGISTERED decode class
-- retired `dis_rule_orderedp_append`, the closure's only consumer. NOT
-- industrialization: no book landed for these — they were waypoint-layer
-- HAND proofs left with zero use sites, so the waypoint-metrics
-- hand-line count moves here as a DELETION, not as coverage.
-- `orderedp_sym` is KEPT deliberately: it is LIVE, read by
-- `orderedp_qsort_native_of_replayed`'s world hypothesis below.)
