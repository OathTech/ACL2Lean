...
dependency's own replayed mirror). The native decode: **the imported FLATTEN
...
/-- The UNCONDITIONAL driver mirror (zero hypotheses — see the coverage row
    `TRUE-LISTP-FLATTEN → REPLAYED ✓`). -/
def trueListpFlattenMirror := driver_mirror% treeDev treeWorldD "true-listp-flatten"
...
theorem true_listp_flatten_native_driver (env : Env) :
  ...
  -- the mirror: (TRUE-LISTP (FLATTEN X)) is eventually truthy
  obtain ⟨N₀, hN₀⟩ := trueListpFlattenMirror env
