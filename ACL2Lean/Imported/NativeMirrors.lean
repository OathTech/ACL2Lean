...
def liftCatalog : List (String × String × LiftStatus) := [
  ...
  ("sorting/ordered-perms", "ORDERED-PERMS", .pending "UNBLOCKED by the dp-premises sub-arc (2026-08-01; supersedes the 2026-07-31 'blocked on emission' probe): all four dp-fact leaves now close in-telescope via the premise machinery (rule-content + equivrefl premises + the trueListp DP-leaf bridge) — the composed cond row carries only rule:CONS-CAR-CDR, rule:ORDEREDP-MEMB, equivrefl:PERM-IS-AN-EQUIVALENCE. The mirror itself is now buildable P3 work (the last pending ordered-perms mirror)"),
