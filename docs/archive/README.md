...
| [`Verify.py`](Verify.py) | **RETIRED 2026-08-19** (Mike's ruling). A standalone Python comparator that shelled out to `acl2`, ran an expression, and diffed the result against ours — an early, hand-driven ancestor of the differential harness. Superseded by `Tests/differential/` (`just diff-test`), which does the same job as a gated corpus with expectation classes. It sat unreferenced at the repo root; kept here as the record, not as a working script. |
...
| [`2026-08-19_symbolfrequency-panic-measurement.md`](2026-08-19_symbolfrequency-panic-measurement.md) | The full measurement record behind the accepted `Lean.LibrarySuggestions.SymbolFrequency` panic: why no project-local switch exists, why a module split cannot work, the deny-list surface that was measured and not taken, and how the panic reads in a gate artifact. RESOLVED: eliminated at v4.31.0+ (upstream #13202), bumped to v4.33.0 2026-08-19 — the resolution postscript is at the end of the doc. Per Mike's 2026-08-19 ruling, resolved caveats carry NO notice in the reader-facing docs; this archive doc is the complete record. |
