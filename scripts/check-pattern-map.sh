...
# capture-halt family: session died before any theorem closed
nosig cov-complex            "(:QED)"
nosig cov-number-literals    "(:QED)"
nosig cov-type-set-inverter  "(:QED)"
# partial-halt family: exactly the earlier theorems closed, then death
sig   cov-quoted-constant-rule 1 "(:QED)"
nosig cov-quoted-constant-rule ":RULE-CLASSES :REWRITE-QUOTED-CONSTANT"
sig   cov-defconst-local     1 ":SOURCE :LOCAL"
sig   cov-defattach          1 "(:QED)"
# crown artifacts
sig cov-force-round   1 "(:QED :FORCED 1)"
sig cov-force-round   1 "[1]Goal"
sig cov-force-round   1 "Modulo the following forced"
